...
```{r}
dat <- read_lines("Calendar.ics") %>% # It's a text file so read it as lines of text
...
```{r}
write_csv(csv_cal, "Calendar.csv") # Write the output to a file
```
...
end_date <- Sys.Date() # An end date...today's date, just to keep it simple
